...
namespace ProjectDawn.Navigation
{
    ...
    {
        [SerializeField] [Tooltip("The size of single cell.")]
        float3 m_CellSize = 3;
    ...
    public partial struct AgentSpatialPartitioningSystem : ISystem
    {
        ...
        public void OnCreate(ref SystemState state)
        {
            ...
            m_Map = new NativeParallelMultiHashMap<int3, int>(
                UseLimitedQuery ? InitialCapacity * MaxCellsPerUnit : InitialCapacity, Allocator.Persistent);
        ...
        [System.Obsolete(
            "This class is obsolete, please use new settings workflow https://lukaschod.github.io/agents-navigation-docs/manual/settings.html.")]
        ...
        public struct Singleton : IComponentData
        {
            ...
            /// </summary>
            public int QuerySphere<T>(float3 center, float radius, ref T action)
                    ...
                    for (int j = min.y; j < max.y; ++j)
                    {
                        for (int k = min.z; k < max.z; ++k)
                        {
                            ...
                            if (m_Map.TryGetFirstValue(new int3(i, j, k), out int index, out var iterator))
                            {
                                do
                                {
                                    action.Execute(m_Entities[index], m_Bodies[index], m_Shapes[index],
                                        m_Transforms[index]);
                                    count++;
                                } while (m_Map.TryGetNextValue(out index, ref iterator));
                            }
            ...
            /// Query agents that intersect with the sphere.
            /// </summary>
            public int QueryCircle<T>(float3 center, float radius, ref T action)
                where T : unmanaged, ISpatialQueryEntity
            {
                int count = 0;
            ...
            /// </summary>
            public int QueryCircle<T>(float3 center, float radius, int maxCount, ref T action)
                where T : unmanaged, ISpatialQueryEntity
            {
                if (maxCount == 0)
                ...
                for (int i = 0; i < maxIterations; i++)
                {
                    ...
                    if ((point.x == point.y) || ((point.x < 0) && (point.x == -point.y)) ||
                        ((point.x > 0) && (point.x == 1 - point.y)))
                    {
                        // Swap and negate x
                        int temp = step.x;
                        step.x = -step.y;
            ...
            /// </summary>
            public int QueryCylinder<T>(float3 center, float radius, float height, ref T action)
                where T : unmanaged, ISpatialQueryEntity
            {
                int count = 0;
            ...
            /// Query agents that intersect with the sphere.
            /// </summary>
            public int QueryCylinder<T>(float3 center, float radius, float height, int maxCount, ref T action)
                where T : unmanaged, ISpatialQueryEntity
            {
                if (maxCount == 0)
                ...
                for (int i = 0; i < maxIterations; i++)
                {
                    ...
                    if (math.all(-halfSize <= point) && math.all(point <= halfSize))
                    {
                        ...
                        {
                            // Find all entities in the bucket
                            if (map.TryGetFirstValue(new int3(offsetedPoint.x, y, offsetedPoint.y), out int index,
                                    out var iterator))
                            {
                                do
                                {
            ...
            /// </summary>
            public int QuerySphereCells<T>(float3 center, float radius, T action)
                where T : unmanaged, ISpatialQueryVolume
            {
                int count = 0;
            ...
            /// </summary>
            public int QueryCylinderCells<T>(float3 center, float radius, float height, T action)
                where T : unmanaged, ISpatialQueryVolume
            {
                int count = 0;
    ...
    partial struct HashJob : IJobEntity
    {
        ...
        void Execute([EntityIndexInQuery] int entityInQueryIndex, in AgentBody body, in AgentShape shape,
