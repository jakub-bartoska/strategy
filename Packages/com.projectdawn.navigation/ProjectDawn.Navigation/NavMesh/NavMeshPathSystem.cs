...
namespace ProjectDawn.Navigation
{
    ...
    public partial struct NavMeshPathSystem : ISystem
    {
        ...
        partial struct NavMeshPathJob : IJobEntity
        {
            ...
            public void Execute(Entity entity, ref DynamicBuffer<NavMeshNode> nodes, ref NavMeshPath path,
                in AgentBody body, in LocalTransform transform)
            {
                switch (path.State)
                {
                    case NavMeshPathState.WaitingNewPath:
                        ...
                        path.EndLocation = NavMesh.MapLocation(body.Destination, path.MappingExtent, path.AgentTypeId,
                            path.AreaMask);
                        path.QueryHandle = NavMesh.CreateQuery(path.Location, path.EndLocation, path.AgentTypeId,
                            path.AreaMask);
