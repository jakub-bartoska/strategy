...
namespace ProjectDawn.Navigation
{
    ...
    public partial struct AgentSonarAvoidSystem : ISystem
    {
        ...
        unsafe partial struct AgentAvoidJob : IJobEntity, IJobEntityChunkBeginEnd
        {
            ...
            public void Execute(Entity entity, ref AgentBody body, in AgentShape shape, in AgentSonarAvoid avoid,
                in LocalTransform transform)
            {
                ...
                {
                    Spatial.QueryCylinder(transform.Position, shape.Radius + sonarRadius, shape.Height,
                        Spatial.m_QueryCapacity, ref action);
                ...
                {
                    Spatial.QueryCircle(transform.Position, shape.Radius + sonarRadius, Spatial.m_QueryCapacity,
                        ref action);
                }
            ...
            struct Action : ISpatialQueryEntity
            {
                    ...
                    LocalTransform otherTransform)
                {
                    ...
                    {
                        if (dot(DesiredDirection, normalizesafe(otherTransform.Position - Transform.Position)) < 0 &&
                            length(otherBody.Velocity) > 0)
                            return;
                    }
                    ...
                    if (Shape.Type == ShapeType.Cylinder && otherShape.Type == ShapeType.Cylinder)
                    {
                        ...
                        if (abs((Transform.Position.y + extent) - (otherTransform.Position.y + otherExtent)) >
                            extent + otherExtent)
                            return;
                    }
