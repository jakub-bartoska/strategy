...
namespace ProjectDawn.Navigation.Editor
{
    ...
    public partial struct AgentAvoidGizmosSystem : ISystem
    {
        ...
        unsafe partial struct AgentAvoidJob : IJobEntity, IJobEntityChunkBeginEnd
        {
            ...
            [ReadOnly] public BufferTypeHandle<NavMeshWall> NavMeshWallHandle;
            [ReadOnly] public BufferLookup<NavMeshWall> NavMeshWallLookup;
            bool HasNavMeshWall;

            public GizmosCommandBuffer.ParallelWriter Gizmos;

            public void Execute(Entity entity, in AgentBody body, in AgentShape shape, in AgentSonarAvoid avoid,
                in LocalTransform transform, in DrawGizmos drawGizmos)
            {
                if (body.IsStopped)
                    return;
                ...
                UnityEngine.Debug.DrawLine(transform.Position, transform.Position + desiredDirection,
                    UnityEngine.Color.black);

#if EXPERIMENTAL_SONAR_TIME
                float sonarRadius =
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             clamp(length(body.Velocity) * 0.9f, 0.1f, min(distance(body.Destination, transform.Position), avoid.Radius));
#else
                float sonarRadius = min(distance(body.Destination, transform.Position), avoid.Radius);
#endif

                // Recreate avoidance structure
                Sonar.Set(transform.Position, body.Velocity, rotation, shape.Radius, sonarRadius,
                    avoid.MaxAngle * 0.5f);
                ...
                if (shape.Type == ShapeType.Cylinder)
                {
                    ...
                    if (HasNavMeshWall)
                    {
                        ...
                        for (int i = 0; i < walls.Length; ++i)
                        {
                            ...
                            if (abs((transform.Position.y + extent) - (wall.Start.y + wallExtent)) >
                                extent + wallExtent)
                                continue;
                ...
                UnityEngine.Debug.DrawLine(transform.Position, transform.Position + newDirection,
            ...
            struct Action : ISpatialQueryEntity
            {
                ...
                public void Execute(Entity otherEntity, AgentBody otherBody, AgentShape otherShape,
                    LocalTransform otherTransform)
                {
                    ...
                    {
                        if (math.dot(DesiredDirection,
                                math.normalizesafe(otherTransform.Position - Transform.Position)) < 0 &&
                            math.length(otherBody.Velocity) > 0)
                            return;
                    }
                    ...
                    if (Shape.Type == ShapeType.Cylinder && otherShape.Type == ShapeType.Cylinder)
                    {
                        ...
                        if (math.abs((Transform.Position.y + extent) - (otherTransform.Position.y + otherExtent)) >
                            extent + otherExtent)
                            return;
                    }
                    ...
                    Sonar.DrawObstacleCircle(DrawCircle, otherTransform.Position, otherBody.Velocity,
                        otherShape.Radius);
                }
            }
            ...
            struct DrawArc : SonarAvoidance.IDrawArc
            {
                ...
                void SonarAvoidance.IDrawArc.DrawArc(float3 position, float3 up, float3 from, float3 to, float angle,
                    UnityEngine.Color color)
                {
                    ...
                    Gizmos.DrawSolidArc(position, up, to, math.degrees(angle), InnerRadius,
                        new UnityEngine.Color(1, 1, 1, 0.4f));
                }
            }
