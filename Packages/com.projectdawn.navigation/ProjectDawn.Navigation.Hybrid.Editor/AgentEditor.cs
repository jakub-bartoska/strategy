...
namespace ProjectDawn.Navigation.Hybrid.Editor
{
    ...
    class AgentEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                if (target is AgentAuthoring auth &&
                    auth.gameObject.GetComponent<UnityEngine.AI.NavMeshAgent>() != null)
                    EditorGUILayout.HelpBox("This component does not work with NavMeshAgent!", MessageType.Error);
                if (Agent.GetComponent<AgentCylinderShapeAuthoring>() == null &&
                    Agent.GetComponent<AgentCircleShapeAuthoring>() == null)
                    EditorGUILayout.HelpBox("It is expected that agent will have Cylinder or Circle shape!",
                        MessageType.Warning);
            }
        ...
        static class Styles
        {
            public static readonly GUIContent MotionType =
                EditorGUIUtility.TrTextContent("Motion Type", "The type of agent motion.");

            public static readonly GUIContent Speed =
                EditorGUIUtility.TrTextContent("Speed", "Maximum movement speed when moving to destination.");

            public static readonly GUIContent Acceleration = EditorGUIUtility.TrTextContent("Acceleration",
                "The maximum acceleration of an agent as it follows a path, given in units / sec^2.");

            public static readonly GUIContent AngularSpeed = EditorGUIUtility.TrTextContent("Angular Speed",
                "Maximum turning speed in (deg/s) while following a path.");

            public static readonly GUIContent StoppingDistance = EditorGUIUtility.TrTextContent("Stopping Distance",
                "Stop within this distance from the target position.");

            public static readonly GUIContent AutoBreaking = EditorGUIUtility.TrTextContent("Auto Breaking",
