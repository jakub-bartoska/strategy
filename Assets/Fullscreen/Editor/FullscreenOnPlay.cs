...
namespace FullscreenEditor
{
 ...
 () => SetIsPlaying(EditorApplication.isPlayingOrWillChangePlaymode && !EditorApplication.isPaused);
        ...
        private static void SetIsPlaying(bool playing)
        {
            ...
            {
                EditorUtility.DisplayDialog("Fullscreen on play conflict",
        ...
        internal static void SetEnterPlayModeBehavior(EditorWindow playmodeView, CustomEnterPlayModeBehavior behaviour)
        {
            if (playmodeView.HasProperty("playModeBehaviorIdx"))
                playmodeView.SetPropertyValue("playModeBehaviorIdx",
                    (int) behaviour); // unity forgot to update this prop when updating the play mode behaviour
