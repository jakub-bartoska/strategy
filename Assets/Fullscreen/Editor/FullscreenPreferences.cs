...
namespace FullscreenEditor
{
    ...
    public static class FullscreenPreferences
    {
        ...
        private static readonly GUIContent resetSettingsContent =
            new GUIContent("Use Defaults", "Reset all settings to default ones");
        ...
        private static readonly PrefItem<Vector2> scroll =
            new PrefItem<Vector2>("Scroll", Vector2.zero, string.Empty, string.Empty);
        ...
        static FullscreenPreferences()
        {
            ...
            ToolbarVisible = new PrefItem<bool>("Toolbar", false, "Toolbar Visible",
                "Show and hide the toolbar on the top of some windows, like the Game View and Scene View.");
            FullscreenOnPlayEnabled = new PrefItem<bool>("FullscreenOnPlay", false, "Fullscreen On Play",
                "Override the \"Maximize on Play\" option of the game view to \"Fullscreen on Play\"");
            RectSource = new PrefItem<RectSourceMode>("RectSource", RectSourceMode.MainDisplay, "Placement source",
                rectSourceTooltip);
            CustomRect = new PrefItem<Rect>("CustomRect", FullscreenRects.GetMainDisplayRect(), "Custom Rect",
                string.Empty);
            DisableNotifications = new PrefItem<bool>("DisableNotifications", false, "Disable Notifications",
                "Disable the notifications that shows up when opening a new fullscreen view.");
            KeepFullscreenBelow = new PrefItem<bool>("KeepFullscreenBelow", true, "Keep Utility Views Above",
            ...
            DisableSceneViewRendering = new PrefItem<bool>("DisableSceneViewRendering", true,
            ...
            UseGlobalToolbarHiding = new PrefItem<bool>("UseGlobalToolbarHiding", FullscreenUtility.IsMacOS,
                "Use global toolbar hiding",
                "Changes toolbars of all windows at once. This option fixes the gray bar bug on MacOS.");
            MosaicMapping = new PrefItem<int[]>("MosaicMapping", new[] {0, 1, 2, 3, 4, 5, 6, 7},
                "Mosaic Screen Mapping", "Defines which display renders on each screen when using Mosaic.");
            DoNotUseWmctrl = new PrefItem<bool>("DoNotUseWmctrl", false, "Do not use wmctrl",
            ...
            if (FullscreenUtility.MenuItemHasShortcut(Shortcut.TOOLBAR_PATH))
                ToolbarVisible.Content.text += string.Format(" ({0})",
            ...
            if (FullscreenUtility.MenuItemHasShortcut(Shortcut.FULLSCREEN_ON_PLAY_PATH))
                FullscreenOnPlayEnabled.Content.text += string.Format(" ({0})",
        ...
        {
            var sp = new SettingsProvider("Preferences/Fullscreen Editor", SettingsScope.User,
        ...
        {
            var sp = new SettingsProvider("Preferences/Fullscreen Editor/Shortcuts", SettingsScope.User,
                contents.Select(c => c.text));
            sp.footerBarGuiHandler = OnFooterGUI;
            sp.guiHandler = (search) =>
            {
        ...
        private static void OnPreferencesGUI(string search)
        {
            ...
            if (RectSource.Value == RectSourceMode.AtMousePosition)
                EditorGUILayout.HelpBox("\'At mouse position\' can cause slowdowns on large projects",
                    MessageType.Warning);

            if (!IsRectModeSupported(RectSource))
                EditorGUILayout.HelpBox("The selected placement source mode is not supported on this platform",
                    MessageType.Warning);
            ...
            if (FullscreenUtility.IsLinux)
            {
                ...
                {
                    EditorGUILayout.HelpBox("'wmctrl' not found. Try installing it with 'sudo apt-get install wmctrl'.",
                        MessageType.Warning);
                }
                else
                {
            ...
            if (FullscreenRects.ScreenCount > 1)
            {
                ...
                for (var i = 0; i < mosaicMapping.Length && i < FullscreenRects.ScreenCount; i++)
                {
                    var val = EditorGUILayout.IntPopup(string.Format("Physical display {0} renders", i + 1),
                        mosaicMapping[i], mosaicDropDownOptions, new[] {0, 1, 2, 3, 4, 5, 6, 7});
                    mosaicMapping[i] = val;
                }
        ...
        private static void OnFooterGUI()
        {
            Func<GUIContent, bool> linkLabel = (label) =>
                typeof(EditorGUILayout).HasMethod("LinkLabel", new[] {typeof(string), typeof(GUILayoutOption[])})
                    ...
                    : // < 2020.1
                    typeof(EditorGUILayout).InvokeMethod<bool>("LinkButton", label,
                        new GUILayoutOption[0]); // >= 2020.1
            ;

            using (new EditorGUILayout.HorizontalScope())
            {
                GUILayout.FlexibleSpace();
                if (linkLabel(new GUIContent("Consider leaving a review if you're enjoying Fullscreen Editor!",
                        REVIEWS_LINK)))
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                {
                    GUI.changed = false;
                    var enable = GUILayout.Toggle(Integration.IsDirectiveDefined("FULLSCREEN_DEBUG"), "Debug",
                        "Button");
                    if (GUI.changed)
                    {
        ...
        private static string GetEmailURL(Exception e = null)
        {
            ...
            Func<string, string> EscapeURL = url =>
                UnityEngine.Networking.UnityWebRequest.EscapeURL(url).Replace("+", "%20");
#else
