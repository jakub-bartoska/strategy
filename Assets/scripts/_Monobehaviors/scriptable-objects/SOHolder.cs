...
namespace _Monobehaviors.scriptable_objects
{
    public class SOHolder : MonoBehaviour
    {
        ...
        private BattalionPosition positionToSoPosition(float3 oldPosition)
        {
            var adjustedPositio = CustomTransformUtils.adjustPositionFromBattleToPreBattle(oldPosition);
        ...
        public NativeList<BattalionToSpawn> getBattalionsToSpawn()
        {
            ...
            foreach (var battalion in debugBattleCompositionSO.battalions)
            {
                result.Add(new BattalionToSpawn
                {
                    ...
                    position = new float3(battalion.positionForSO.x, battalion.positionForSO.y,
                        battalion.positionForSO.z)
                });
            }
